...
public class DragonAnimationContext {
    ...
    public Vec3 getThroatPosition(double offsetX, double offsetY, double offsetZ) {
        return this.getThroatPosition(/*this.model.head*/null, offsetX, offsetY, offsetZ);
    }
     ...
     */
    public Vec3 getThroatPosition(Object object, double offsetX, double offsetY, double offsetZ) {
       /* final float scale = this.dragon.getScale();
        ...
        return new Vec3(-head.pivotX * modelScale, -head.pivotY * modelScale, (head.pivotZ - 15D) * modelScale)
                // offset of the throat position relative to the head origin - rotate and pitch to match head
                .add(new Vec3(offsetX * headScale, (offsetY + 2D) * headScale, (offsetZ - 8D) * headScale).rotateX(head.pitch).rotateY(-head.yaw))
    ...
    */
    public void tick(boolean firstTick) {
        ...
        this.animTimer.add(onGround ? 0.035F : 0.07F - Mth.clamp(speedEnt / speedMax, 0, 1) * 0.035F);
...
*/
    public float getModelPitch(float partialTicks) {
        ...
        return clampedSmoothLinear(60F, Mth.clamp(yTrail.get(partialTicks, 5, 0) * 10F, -90F, 90F), speed);
    }
    ...
    public float getRelativeHeadSize(float scale) {
        ...
        scale = Mth.clamp(scale, SCALE_OF_BABY, SCALE_OF_ADULT);
