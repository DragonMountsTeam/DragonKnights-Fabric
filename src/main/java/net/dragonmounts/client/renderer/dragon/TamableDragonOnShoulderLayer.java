...
//
//public class TamableDragonOnShoulderLayer<T extends PlayerEntity> extends RenderLayer<T, PlayerEntityModel<T>> {
...
//    }
//
//    public void render(CompoundTag tag, MatrixStack matrices, VertexConsumerProvider buffers, int light, double offsetX, double offsetY, double offsetZ) {
//        if (EntityType.getId(DMEntities.TAMEABLE_DRAGON).toString().equals(tag.getString("id"))) {
//            DragonModel model = this.model;
//            DragonVariant variant = DragonVariant.REGISTRY.get(new ResourceLocation(tag.getString(DragonVariant.DATA_PARAMETER_KEY)));
